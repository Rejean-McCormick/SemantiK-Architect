...
class Realizer:
    async def realize(self, plan: ConstructionPlan) -> SurfaceResult:
        return SurfaceResult(
            text="Alan Turing is a British mathematician.", lang_code=plan.lang_code,
...
@pytest.mark.asyncio
async def test_generate_text_english_is_planner_first() -> None:
    frame = BioFrame(frame_type="bio", subject={"name":"Alan Turing","profession":"mathematician","nationality":"British"})
    result = await GenerateText(realizer=Realizer(), capabilities=Caps()).execute("en", frame)
